...
export default function EscanerScreen({ route, navigation }) {
  ...
  const [progresoLocal, setProgresoLocal] = useState(
    auditoriaContext?.escaneados || 0,
  );
  const metaTotal = auditoriaContext?.total || 0;

  // Domain layer
  const { lookupActivo, submitScannedActivo, isLoading, error, setError } =
    useEscaner();
  ...
  const handleQRScanned = async (scanningResult) => {
    if (escaneado) return;
    const { data, bounds, cornerPoints } = scanningResult;
    ...
    if (cornerPoints && cornerPoints.length > 0) {
      ...
      qrCenterX = bounds.origin.x + (bounds.size?.width || 0) / 2;
      qrCenterY = bounds.origin.y + (bounds.size?.height || 0) / 2;
    }

    if (qrCenterX === undefined || qrCenterY === undefined) {
      return;
    }
    ...
    // OBTENIDO POR CALIBRACIÓN EMPÍRICA CON EL USUARIO REAL:
    // EJE Y (Vertical): Comprobado normal. Arriba=30%, Centro=55%, Abajo=75%.
    const minY = screenHeight * 0.45;
    const maxY = screenHeight * 0.65;

    // EJE X (Horizontal): ¡Comprobado de reversa!
    // Al usar 55% nos acercamos muchísimo, pero aún tiraba un poco a la derecha.
    // Sabiendo que la Derecha es 40%, empujar la mira más a la Izquierda requiere valores más altos.
    // Subimos de 65% a 80% para clavar exactamente la aguja en el verdadero centro-visual.
    ...
    try {
      ...
      if (
        auditoriaContext &&
        activoData.ubicacion !== auditoriaContext.ubicacion_nombre
      ) {
        Alert.alert(
      ...
      if (normalizedState.includes("funcional"))
        setEstadoSeleccionado("funcional");
      else if (normalizedState.includes("mantenimiento"))
        setEstadoSeleccionado("mantenimiento");
      ...
      Alert.alert(
        "Error",
        err.message || "Activo no encontrado en base de datos.",
        [
          {
            text: "Reintentar",
            onPress: () => setTimeout(() => setEscaneado(false), 2000),
          },
        ],
      );
    }
  ...
  const procesarGuardado = async () => {
    ...
    try {
      const payload = {
        ...
        nota_historial:
          observacion || "Confirmado mediante Auditoría S.G.A.F.A.",
      };

      await submitScannedActivo(
        activoEscaneado.id,
        payload,
        auditoriaContext?.id,
      );
      ...
      Alert.alert(
        "Error de Red",
        err.message || "No se pudo actualizar el activo.",
      );
    }
  };
  ...
  const procesarFinalizacion = async () => {
    if (!resumenFinal.trim()) {
      Alert.alert(
        "Advertencia",
        "Por favor brinda una descripción muy breve o el estado general detectado.",
      );
      return;
    }
    ...
    try {
      ...
      Alert.alert(
        "Misión Cumplida",
        "¡Auditoría cerrada con éxito! La Web administrativa ha sido notificada al instante.",
        [
          {
            text: "Terminar",
            onPress: () => {
  ...
  return (
    <View style={styles.container}>
        ...
        <View style={styles.headerLeft}>
          ...
          <View>
            <Text style={styles.headerTitle}>
              {auditoriaContext ? "MODO AUDITORÍA" : "ESCÁNER RÁPIDO"}
            </Text>
            <Text style={styles.headerSubtitle}>
              {auditoriaContext
                ? `Misión: ${auditoriaContext.folio}`
                : "Módulo de Inspección Local"}
            </Text>
          </View>
        </View>
      </View>
      ...
      {!modalFinalVisible && (
        <View
          style={
            auditoriaContext ? styles.cameraShrink : StyleSheet.absoluteFill
          }
        >
          <CameraView
          ...
          />
          {/* Overlay desasociado de CameraView para prevenir crasheos en Expo 50+ */}
          <View
          ...
          >
            <View style={styles.frameWrapper} pointerEvents="none">
              ...
              <Text style={styles.hint}>
                {isLoading
                  ? "Buscando activo..."
                  : "Alinea el código QR del activo"}
              </Text>
            </View>
            ...
            >
              ...
              <TouchableOpacity
                style={[
                  ...
                  (!estadoSeleccionado || isLoading) && styles.disabledButton,
                ]}
              ...
              >
                {isLoading ? (
                  ...
                  <Text style={styles.primaryButtonText}>
                    {progresoLocal + 1 >= metaTotal && auditoriaContext
                      ? "Guardar y Finalizar Misión"
          ...
          <View style={styles.modalContentRounded}>
            <View style={{ alignItems: "center", marginBottom: 20 }}>
              ...
              <Text style={styles.successSubtitleFinal}>
                Se han contabilizado los {metaTotal} activos físicos
                exitosamente en {auditoriaContext?.ubicacion_nombre}.
              </Text>
            </View>
...
const getStyles = (colors, isDark) =>
  StyleSheet.create({
    ...
    successIconFinal: {
      ...
      height: 60,
      borderRadius: 30,
      backgroundColor: isDark ? "rgba(22, 101, 52, 0.3)" : colors.successBg,
      justifyContent: "center",
      alignItems: "center",
      marginBottom: 12,
    },

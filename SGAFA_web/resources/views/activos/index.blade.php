...
// Datos completos de activos embebidos desde PHP
const ACTIVOS_DATA = @json($activosData);
...
function abrirPanel(modo, idx) {
    ...
    if (modo === 'nuevo') {
        ...
        footer.innerHTML = `
            ...
            <button class="btn-primary" style="padding:9px 20px;" onclick="guardarNuevo()">
        ...
        footer.innerHTML = `
            <button class="btn-outline" style="padding:9px 16px;color:#dc2626;border-color:#fecaca;" onclick="confirmarEliminar('${a.codigo}','${a.nombre}')">
            ...
            <div style="display:flex;gap:8px;margin-left:auto;">
                <button class="btn-outline" style="padding:9px 20px;" onclick="cerrarPanel()">Cancelar</button>
                <button class="btn-primary" style="padding:9px 20px;" onclick="guardarEditar(${idx})">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2.2" viewBox="0 0 24 24"><path d="M19 21H5a2 2 0 01-2-2V5a2 2 0 012-2h11l5 5v11a2 2 0 01-2 2z"/><polyline points="17 21 17 13 7 13 7 21"/><polyline points="7 3 7 8 15 8"/></svg>
                    Guardar cambios
                </button>
            </div>`;
...
function guardarNuevo() {
    ...
    const usuarioRaw= document.getElementById('nUsuario').value;
    const usuario   = usuarioRaw === 'Sin asignar' ? '—' : usuarioRaw;
    ...
    const colorMap = { 'Funcional':'blue', 'Mantenimiento':'yellow', 'Baja':'green' };
    const color    = colorMap[estado] || 'blue';
    const nuevoActivo = { codigo, nombre, categoria, ubicacion, usuario, estado, color, foto: fotoTempNuevo || null };

    ACTIVOS_DATA.push(nuevoActivo);
    const idx = ACTIVOS_DATA.length - 1;

    const tbody = document.querySelector('tbody');
    const tr = document.createElement('tr');
    tr.innerHTML = renderFila(nuevoActivo, idx);
...
// --- GUARDAR EDITAR ---
function guardarEditar(idx) {
    const nombre    = document.getElementById('eNombreVal').value.trim();
    const categoria = document.getElementById('eCategoria').value;
    const ubicacion = document.getElementById('eUbicacion').value.trim();
    ...
    const colorMap = { 'Funcional':'blue', 'Mantenimiento':'yellow', 'Baja':'green' };
    ACTIVOS_DATA[idx].nombre    = nombre;
    ACTIVOS_DATA[idx].categoria = categoria;
    ACTIVOS_DATA[idx].ubicacion = ubicacion;
    ACTIVOS_DATA[idx].estado    = estado;
    ACTIVOS_DATA[idx].usuario   = usuario;
    ACTIVOS_DATA[idx].color     = colorMap[estado] || 'blue';
    if (fotoTempEdit) ACTIVOS_DATA[idx].foto = fotoTempEdit;

    const rows = document.querySelectorAll('tbody tr');
    if (rows[idx]) rows[idx].innerHTML = renderFila(ACTIVOS_DATA[idx], idx);
...
// --- RENDER FILA TABLA ---
function renderFila(a, idx) {
    const bc = { blue:'#dbeafe', yellow:'#fef9c3', green:'#dcfce7' };
    const bt = { blue:'#1d4ed8', yellow:'#a16207', green:'#15803d' };
    const dot= { blue:'#3b82f6', yellow:'#ca8a04', green:'#16a34a' };
    const bg = bc[a.color]||'#f0f2f5', fg=bt[a.color]||'#374151', dc=dot[a.color]||'#6b7280';
    return `
        ...
        <td><span style="background:${bg};color:${fg};font-size:.78rem;font-weight:700;padding:3px 10px;border-radius:20px;display:inline-flex;align-items:center;gap:5px;"><span style="width:6px;height:6px;border-radius:50%;background:${dc};display:inline-block;"></span>${a.estado}</span></td>
        <td>
            <div style="display:flex;gap:6px;align-items:center;">
                <button class="action-btn" title="Ver detalle" onclick="abrirPanel('ver',${idx})">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                </button>
                <button class="action-btn" title="Editar" onclick="abrirPanel('editar',${idx})">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M11 4H4a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 013 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button class="action-btn" title="Ver QR" style="color:#4a86b5;border-color:#bfdbfe;" onclick="mostrarQR('${a.codigo}','${a.nombre}')">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><path d="M14 14h3v3m0 4h4m-4 0v-4m4 0h-4"/></svg>
                </button>
                <button class="action-btn" title="Eliminar" onclick="confirmarEliminar('${a.codigo}','${a.nombre}')" style="color:#dc2626;border-color:#fecaca;">
                    <svg width="13" height="13" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 01-2 2H8a2 2 0 01-2-2L5 6"/><path d="M10 11v6M14 11v6"/><path d="M9 6V4a1 1 0 011-1h4a1 1 0 011 1v2"/></svg>
                </button>
            </div>
        </td>`;
...
function renderFormNuevo() {
    ...
    <div class="pf-row">
        <div><label class="pf-label">Nombre del activo *</label><input id="nNombre" class="pf-field" type="text" placeholder="Ej. Escritorio" autofocus></div>
        <div><label class="pf-label">Código</label><input id="nCodigo" class="pf-field" type="text" value="${nextCodigo()}" readonly style="background:#f8fafc;color:#6b7a8d;cursor:not-allowed;"></div>
    </div>
    <div class="pf-row">
        <div><label class="pf-label">Categoría</label>
            <select id="nCategoria" class="pf-field">
                <option>Bienes muebles</option>
                <option>Bienes de no uso</option>
                <option>Bienes de menor valor</option>
            </select></div>
        <div><label class="pf-label">Ubicación</label><input id="nUbicacion" class="pf-field" type="text" placeholder="Oficina / Área"></div>
    </div>
    <div class="pf-row">
        <div><label class="pf-label">Estado</label>
            <select id="nEstado" class="pf-field">
                <option>Funcional</option><option>Mantenimiento</option><option>Baja</option>
            </select></div>
        <div><label class="pf-label">Usuario asignado</label>
            <select id="nUsuario" class="pf-field">
                <option>Sin asignar</option><option>Santiago Meneses</option><option>Carlos Chavez</option>
                <option>Ruth Piña</option><option>Andre Martinez</option><option>Hannia Gonzales</option><option>Valeria Briones</option>
            </select></div>
    </div>
    ...
    <div class="pf-group">
        <div id="fotoDropZone" onclick="document.getElementById('fotoInput').click()"
             style="border:2px dashed #d1d5db;border-radius:12px;padding:24px 16px;text-align:center;cursor:pointer;background:#fafafa;transition:border-color .2s;"
             onmouseenter="this.style.borderColor='#4a86b5'" onmouseleave="this.style.borderColor='#d1d5db'">
            <div id="fotoPreviewWrap" style="display:none;margin-bottom:8px;">
                <img id="fotoPreview" src="" style="max-height:160px;max-width:100%;border-radius:10px;object-fit:cover;">
            </div>
            <div id="fotoPlaceholder">
                <svg width="28" height="28" fill="none" stroke="#9ca3af" stroke-width="1.6" viewBox="0 0 24 24" style="margin:0 auto 8px;display:block;"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                <p style="font-size:.83rem;color:#6b7a8d;margin:0;">Haz clic para seleccionar una foto</p>
                <p style="font-size:.75rem;color:#b0b8c1;margin:4px 0 0;">JPG, PNG o WEBP — máx. 5 MB</p>
            </div>
        </div>
        <input id="fotoInput" type="file" accept="image/*" style="display:none;" onchange="handleFotoNuevo(this)">
    </div>
    <div style="display:flex;align-items:center;gap:8px;padding:10px 14px;background:#eff6ff;border-radius:10px;margin-top:4px;">
        ...
        <span style="font-size:.8rem;color:#4a86b5;font-weight:600;">El código QR se generará automáticamente al guardar el activo.</span>
    </div>
    <div class="pf-group" style="margin-top:12px;"><label class="pf-label">Descripción</label><textarea id="nDesc" class="pf-field" rows="3" placeholder="Observaciones del activo..."></textarea></div>`;
}
...
function renderFormEditar(a) {
    ...
    <div class="pf-section">Información general</div>
    <div class="pf-row">
        <div><label class="pf-label">Código</label><input id="eNombre" class="pf-field" type="text" value="${a.codigo}" readonly></div>
        <div><label class="pf-label">Nombre *</label><input id="eNombreVal" class="pf-field" type="text" value="${a.nombre}"></div>
    </div>
    <div class="pf-row">
        <div><label class="pf-label">Categoría *</label>
            <select id="eCategoria" class="pf-field">${opts(['Bienes muebles','Bienes de no uso','Bienes de menor valor'], a.categoria)}</select></div>
        <div><label class="pf-label">Ubicación *</label>
            <input id="eUbicacion" class="pf-field" type="text" value="${a.ubicacion}"></div>
    </div>
    <div class="pf-row">
        <div><label class="pf-label">Estado *</label>
            <select id="eEstado" class="pf-field">${opts(['Funcional','Mantenimiento','Baja'], a.estado)}</select></div>
        <div><label class="pf-label">Usuario asignado</label>
            <select id="eUsuario" class="pf-field">${opts(['—','Santiago Meneses','Carlos Chavez','Ruth Piña','Andre Martinez','Hannia Gonzales','Valeria Briones'], a.usuario)}</select></div>
    </div>
    <div class="pf-section" style="margin-top:8px;">Foto del activo</div>
    <div class="pf-group">
        <div id="fotoDropZoneEdit" onclick="document.getElementById('fotoInputEdit').click()"
             style="border:2px dashed #d1d5db;border-radius:12px;padding:${tieneFoto?'10px':'24px'} 16px;text-align:center;cursor:pointer;background:#fafafa;transition:border-color .2s;"
             onmouseenter="this.style.borderColor='#4a86b5'" onmouseleave="this.style.borderColor='#d1d5db'">
            <div id="fotoPreviewWrapEdit" style="display:${tieneFoto?'block':'none'};margin-bottom:8px;">
                <img id="fotoPreviewEdit" src="${a.foto||''}" style="max-height:160px;max-width:100%;border-radius:10px;object-fit:cover;">
            </div>
            <div id="fotoPlaceholderEdit" style="display:${tieneFoto?'none':'block'};">
                <svg width="28" height="28" fill="none" stroke="#9ca3af" stroke-width="1.6" viewBox="0 0 24 24" style="margin:0 auto 8px;display:block;"><rect x="3" y="3" width="18" height="18" rx="3"/><circle cx="8.5" cy="8.5" r="1.5"/><polyline points="21 15 16 10 5 21"/></svg>
                <p style="font-size:.83rem;color:#6b7a8d;margin:0;">Haz clic para cambiar la foto</p>
                <p style="font-size:.75rem;color:#b0b8c1;margin:4px 0 0;">JPG, PNG o WEBP — máx. 5 MB</p>
            </div>
            ${tieneFoto ? `<p style="font-size:.75rem;color:#9ca3af;margin:6px 0 0;">Haz clic para cambiar la foto</p>` : ''}
        </div>
        <input id="fotoInputEdit" type="file" accept="image/*" style="display:none;" onchange="handleFotoEdit(this)">
    </div>
    <div class="pf-group" style="margin-top:12px;"><label class="pf-label">Descripción</label><textarea id="eDesc" class="pf-field" rows="3" placeholder="Observaciones del activo..."></textarea></div>`;
}

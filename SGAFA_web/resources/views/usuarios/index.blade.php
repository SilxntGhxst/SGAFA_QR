...
<div class="card">
    <table class="data-table">
        ...
        <tbody id="usuariosBody">
            ...
            <tr class="usuario-row" data-nombre="{{ strtolower(($u['nombre']??'').' '.($u['apellidos']??'')) }}" data-email="{{ strtolower($u['email']??'') }}">
                ...
                <td style="font-size:.85rem;color:#6b7a8d;">{{ $u['email'] }}</td>
            ...
            @empty
            <tr><td colspan="5" style="text-align:center;padding:24px;color:#9ca3af;font-size:.9rem;">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;padding:14px 20px;border-top:1px solid #f0f2f5;">
        <span style="font-size:.85rem;color:#6b7a8d;">{{ $total ?? count($usuarios) }} usuario(s) registrado(s)</span>
    </div>
</div>
...
<div class="modal-overlay" id="modal-delete" style="z-index:1001;">
    <div class="modal" style="max-width:400px;">
        <div style="padding:28px;">
            ...
            <h3 style="font-family:'Sora',sans-serif;font-size:1.05rem;font-weight:800;color:#0f1f35;margin-bottom:6px;">¿Eliminar usuario?</h3>
            <p id="deleteDesc" style="font-size:.88rem;color:#6b7a8d;line-height:1.5;margin-bottom:20px;">Esta acción no se puede deshacer.</p>
...
.modal { background:#fff;border-radius:16px;width:520px;max-width:95vw;max-height:90vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,.18);opacity:0;transform:translateY(16px) scale(.98);transition:transform .22s,opacity .22s; }
.modal-overlay.open .modal { opacity:1;transform:translateY(0) scale(1); }
.modal-header { padding:22px 24px 18px;border-bottom:1px solid #f0f2f5;display:flex;align-items:center;justify-content:space-between;gap:12px; }
.modal-header h2 { font-size:1.05rem;font-weight:700;color:#1a2535;margin:0; }
.modal-subtitle { font-size:.8rem;color:#9aa5b4;margin-top:2px; }
.modal-close { width:32px;height:32px;border-radius:8px;border:1px solid #e8ecf0;background:#fff;display:grid;place-items:center;cursor:pointer;color:#6b7a8d;flex-shrink:0;transition:background .15s; }
.modal-close:hover { background:#f7f9fc;color:#2d3748; }
.modal-body { padding:22px 24px; }
.modal-footer { padding:16px 24px;border-top:1px solid #f0f2f5;display:flex;justify-content:flex-end;gap:10px; }
.modal-section-label { font-size:.72rem;font-weight:700;text-transform:uppercase;letter-spacing:.6px;color:#9aa5b4;margin-bottom:14px; }
...
.modal-form-group label { font-size:.8rem;font-weight:600;color:#4a5568;letter-spacing:.2px; }
.modal-input { width:100%;border:1.5px solid #e2e8f0;border-radius:8px;padding:9px 12px;font-size:.875rem;font-family:inherit;color:#2d3748;background:#fff;outline:none;transition:border-color .15s,box-shadow .15s;box-sizing:border-box; }
...
.modal-btn-secondary { background:#fff;color:#4a5568;border:1.5px solid #e2e8f0;padding:9px 18px;border-radius:8px;font-size:.85rem;font-weight:600;font-family:inherit;cursor:pointer;transition:background .15s; }
.modal-btn-secondary:hover { background:#f7f9fc; }
